...
from .utils import (get_student, get_product,
                    get_purchase_status,
                    create_purchase_status_obj)
...
def add_student_to_product(request):
    ...
    По проверке статуса оплаты проверка, что
    студент не состоит в группе. Добавление
    студента в группу"""
    ...
    if request.method == 'POST':
        ...
        if student_form.is_valid():
            ...
            if not product_obj or not student_obj:
                context = {'error': 'Студента или продукта \
            ...
            if purchased_status_obj:
                context = {'error': 'Пользователь по данному \
                            курсу уже состоит в группе!',
                           'form': AddStudentForm()}
                return render(request, 'users/add_student.html', context)
            purchased_status_obj = create_purchase_status_obj(
                    student_obj=student_obj, product_obj=product_obj)
